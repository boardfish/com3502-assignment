...
\begin{figure}[H]
  \begin{center}
    \frame{\includegraphics[width=0.6\textwidth]{question1.png}}
  \end{center}
\end{figure}
\end{mdframed}
\vspace*{\baselineskip}
...
\\
\begin{mdframed}
The amplitude varies between +/- 0.005 with these settings. By adjusting the depth, the range reaches the boundaries of +/- 0.01. When the depth is 0, the amplitude remains at 0.
\begin{figure}[H]
  \begin{center}
    \frame{\includegraphics[width=0.6\textwidth]{lfo1.png}}
  \end{center}
\end{figure}
\begin{figure}[H]
  \begin{center}
    \frame{\includegraphics[width=0.6\textwidth]{lfo2.png}}
  \end{center}
\end{figure}
\end{mdframed}
\vspace*{\baselineskip}
...
\\
\begin{mdframed}
The given speech sample activates the lower end of the spectrum - it's transmitting low-pitch frequencies that, if shifted any lower, will result in loss of data - particularly those of voiced sounds that come through at these lower frequencies.
\end{mdframed}
\vspace*{\baselineskip}
...
\\
\begin{mdframed}
When the feedback value of the sound reaches 1, the audio cannot be heard clearly, only the current sound (howling). When the acoustic feedback reaches 1, the sound will resonate and cause howling.
\end{mdframed}
\vspace*{\baselineskip}
...
\\
\begin{mdframed}
I developed our voice changer as its own patch using pd-vanilla on Linux. It can be launched as \texttt{VoiceChanger-help.pd}. I'm hoping it should work out of the box on pd-extended, but I used pd-vanilla with ELSE during development. I was able to include many features I'd worked on in earlier sections of the assignment as abstractions - frequency shift, delay/feedback, and vibrato are all present. Additionally, I built a gate abstraction, which silences the input at regular intervals. Voice input via \texttt{[adc]} or pre-recorded input from \texttt{speech.wav} can be selected by supplying the \texttt{voice} or \texttt{recording} arguments respectively to \texttt{[VoiceChanger]}. I wasn't able to get some elements working, namely tremolo and flanger, due to some issues developing with the LFO. \begin{figure}[H]
  \begin{center}
    \frame{\includegraphics[width=0.6\textwidth]{voicechanger.png}}
  \end{center}
\end{figure}
\end{mdframed}
\vspace*{\baselineskip}
